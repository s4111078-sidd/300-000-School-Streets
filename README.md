...
│       └── charts/              ← Chart PNGs copied from outputs/
│
...
```bash
python prepare_data.py          # bundles pipeline outputs → docs/data/data.json + copies charts
cd docs && python -m http.server 8080   # serve locally
# open http://localhost:8080 in your browser
```
`prepare_data.py` must be run after any pipeline step that changes outputs. It pre-computes all 30 scenario results (10 interventions × 3 schools) so the dashboard runs fully client-side with no backend.
...
An interactive static dashboard hosted on GitHub Pages. Runs entirely client-side — no backend or server required in production.

### Setup
```bash
python prepare_data.py           # generate docs/data/data.json + copy charts (run after pipeline)
cd docs && python -m http.server 8080    # local preview
```
...
|---|---|
| **Hero** | Project summary with 5 live stats: schools assessed, major hazards, Darebin crash count, equity correlation, peak crash hour |
| **Map** | Leaflet map — school markers coloured by severity (red pulse = Major), popup with score + "View School Details" link |
| **School Assessments** | Per-school tab: radar chart, HS indicator bars, overall score, key hazard box |
| **What-if Scenario Analysis** | Select a school + intervention → instant before/after HS chart + severity/cost/timeframe summary. 30 pre-computed results (10 × 3 schools) |
| **Data Analysis** | HS charts, ML MAE bar chart, SEIFA disadvantage cards, equity chart, crash trends chart, demographics chart |
| **Prioritised Interventions** | Filterable table (school + priority), CSV download |
| **About** | Healthy Streets framework explainer, 10-indicator table, severity classification rules |

### `prepare_data.py` — what it does

| Output | Source | Description |
|---|---|---|
| `docs/data/data.json` | All pipeline CSVs | Schools, HS scores, ML results, SEIFA, recommendations, scenario results, hero stats |
| `docs/data/charts/*.png` | `outputs/*.png` | 9 chart images copied for the dashboard |

**Scenario pre-computation:** runs `src.scenarios.engine.run_scenario()` for all 10 interventions × 3 schools and embeds results in `data.json`. The dashboard reads these instantly — no Python runtime needed in the browser.
...
| `remove_arterial` | Reroute heavy vehicles / reduce arterial traffic | High / long-lead | 2–5 years | HS5 |
...
| `prepare_data.py` | `pandas`, `numpy` (+ `scikit-learn` for scenarios) |
